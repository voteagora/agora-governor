...
library Validator {
    ...
    function determineValidation(
        ...
        IMiddleware.SupportedTypes supportedType,
        IMiddleware.Comparators comparison
    ) public pure {
        if (supportedType == IMiddleware.SupportedTypes.UINT8) {
            validate_uint8(param, scopedParam, comparison);
        }

        if (supportedType == IMiddleware.SupportedTypes.UINT16) {
            validate_uint16(param, scopedParam, comparison);
        }

        if (supportedType == IMiddleware.SupportedTypes.UINT32) {
            validate_uint32(param, scopedParam, comparison);
        }

        if (supportedType == IMiddleware.SupportedTypes.UINT64) {
            validate_uint64(param, scopedParam, comparison);
        }

        if (supportedType == IMiddleware.SupportedTypes.UINT128) {
            validate_uint128(param, scopedParam, comparison);
        }

        if (supportedType == IMiddleware.SupportedTypes.UINT256) {
            validate_uint256(param, scopedParam, comparison);
        }

        if (supportedType == IMiddleware.SupportedTypes.ADDRESS) {
            validate_address(param, scopedParam, comparison);
        }

        if (supportedType == IMiddleware.SupportedTypes.BYTES32) {
     ...
     */
    function validate_uint32(bytes calldata param, bytes calldata scopedParam, IMiddleware.Comparators comparison)
        internal
        pure
    {
        compare(
     ...
     */
    function validate_uint64(bytes calldata param, bytes calldata scopedParam, IMiddleware.Comparators comparison)
        internal
        pure
    {
        compare(
     ...
     */
    function validate_uint128(bytes calldata param, bytes calldata scopedParam, IMiddleware.Comparators comparison)
        internal
        pure
    {
        compare(
     ...
     */
    function validate_uint256(bytes calldata param, bytes calldata scopedParam, IMiddleware.Comparators comparison)
        internal
        pure
    {
        compare(
     ...
     */
    function validate_address(bytes calldata param, bytes calldata scopedParam, IMiddleware.Comparators comparison)
        internal
        pure
    {
     ...
     */
    function validate_bytes32(bytes calldata param, bytes calldata scopedParam, IMiddleware.Comparators comparison)
        internal
        pure
    {
