...
interface IHooks {
    ...
    function beforeVoteSucceeded(address sender, uint256 proposalId) external view returns (bytes4, bool, bool);
    ...
    function afterQuorumCalculation(address sender, uint256 proposalId, uint256 quorum) external returns (bytes4);
    ...
    function beforeQueue(
        ...
        bytes[] memory calldatas,
        bytes32 descriptionHash
    ) external returns (bytes4, address[] memory, uint256[] memory, bytes[] memory, bytes32);
